...
static struct PCI_DISPLAY_INFO_T {
    uint16_t display_id;
    const char *name;
} display_info [] = {
...
prepare_display_buffer (int type) 
{
    ...
    u8 temp;
    struct PCI_DISPLAY_INFO_T *pci_info;
    int ii = 0;
    ...
    len += snprintf (display_buffer.buf + len, BUF_SZ - len,
                     "PCI Section:\n");
    for (cur_dev = pci_get_device(PCI_ANY_ID, PCI_ANY_ID, NULL);
         cur_dev;
         cur_dev = pci_get_device(PCI_ANY_ID, PCI_ANY_ID, cur_dev)) {

        len += snprintf (display_buffer.buf + len, BUF_SZ - len,
                         "#################################\n");
        for (ii = 0, pci_info = &display_info[ii]; 
             display_info[ii].name;
             pci_info = &display_info[++ii]) {
            pci_read_config_byte(cur_dev, pci_info->display_id, &temp);
            len += snprintf (display_buffer.buf + len, BUF_SZ - len,
                             "\t%s = %d\n", pci_info->name, temp);
        }
    }
